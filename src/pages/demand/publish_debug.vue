    ...
    <view class="page-header-unified">
      <view class="page-header-content">
        <view class="header-left" @tap="goBack">
          <uni-icons type="back" size="20" color="#F5F1E8"></uni-icons>
        </view>
        <text class="page-header-title">发布需求</text>
...
const remoteModes = [
  ...
  { value: true, label: '仅远程' },
  { value: false, label: '仅非远程' },
]
...
const inferProjectCycle = (durationText: string, rawText: string): string => {
  ...
  if (/半年|6个月/i.test(s)) return 'LONG'
  if (/\d+\s*周/i.test(s) || /短期/i.test(s) || /三个月内/i.test(s)) return 'SHORT'
  return ''
}
